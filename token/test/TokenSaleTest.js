...
contract('TokenSale', function(accounts) {
    ...
    beforeEach(async () => {
        ...
        contractInstance = await TokenSaleContract.new(tokenContractInstance.address, tokenPrice);
        await tokenContractInstance.transfer(contractInstance.address, numTokenForInitialSale, {from: admin});
    });
    ...
    describe('Token buying', function () {
        it('should keep track of token sold correctly', async function () {
            await contractInstance.buyTokens(numTokens, {from: buyer, value: value});
        ...
        it('should emit the sell event', async function () {
            const receipt = await contractInstance.buyTokens(numTokens, {from: buyer, value: value});
            truffleAssert.eventEmitted(receipt, 'Sell', (ev) => {
                ev._buyer.should.equals(buyer);
                return ev._amount.should.be.a.bignumber.that.equals(numTokens.toString());
            });
        });

        it('should enforce the right price', async function () {
            await truffleAssert.fails(
                contractInstance.buyTokens(numTokens, {from: buyer, value: value + 1}),
        ...
        it('should enforce the contract has enough tokens', async function () {
            ...
            await truffleAssert.fails(
                contractInstance.buyTokens(numTokensToBuy, {from: buyer, value: numTokensToBuy * tokenPrice }),
                truffleAssert.ErrorType.REVERT
            );
        });
